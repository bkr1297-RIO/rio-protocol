...
const jsonFiles = [
  'manifest.json',
  'non_collapse_bundle_provenance_v0.1.json',
  'tests/non_collapse/non_collapse_vectors_v0.1.json',
  'tests/non_collapse/non_collapse_evaluation_record.schema.json',
  'tests/non_collapse/non_collapse_failure_taxonomy_v0.1.json',
  'tests/non_collapse/non_collapse_matcher_rules_v0.1.json',
  'tests/non_collapse/non_collapse_failure_record.schema.json',
  'tests/non_collapse/non_collapse_golden_summary.schema.json',
  'tests/non_collapse/non_collapse_golden_summary.example.json',
  'tests/non_collapse/non_collapse_harness_contract.json',
  'tests/non_collapse/non_collapse_pass_fail_algorithm.json',
  'tests/non_collapse/non_collapse_runtime_trace.schema.json',
  'tests/non_collapse/non_collapse_conformance_report.schema.json',
  'tests/non_collapse/non_collapse_conformance_report.example.json',
  'tests/non_collapse/non_collapse_patch_target_matrix_v0.1.json',
  'tests/non_collapse/MANNY_RUNTIME_TRACE_BRIEF.json'
];
...
const jsonlFiles = [
  'tests/non_collapse/non_collapse_variants_v0.1.jsonl',
  'tests/non_collapse/non_collapse_golden_runs_v0.1.jsonl',
  'tests/non_collapse/non_collapse_negative_runs_v0.1.jsonl'
];
...
if (manifest) {
  for (const artifact of manifest.artifacts) {
    const exists = existsSync(resolve(root, artifact.path));
...
const prov = parseJSON(resolve(root, 'non_collapse_bundle_provenance_v0.1.json'));
if (prov) {
...
const algo = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_pass_fail_algorithm.json'));
if (algo) {
...
const contract = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_harness_contract.json'));
if (contract) {
...
const matrix = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_patch_target_matrix_v0.1.json'));
if (matrix) {
...
const evalSchema = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_evaluation_record.schema.json'));
if (evalSchema) {
...
const failSchema = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_failure_record.schema.json'));
if (failSchema) {
...
const traceSchema = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_runtime_trace.schema.json'));
if (traceSchema) {
...
const reportSchema = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_conformance_report.schema.json'));
if (reportSchema) {
...
const goldenExample = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_golden_summary.example.json'));
const goldenSchema = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_golden_summary.schema.json'));
...
const reportExample = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_conformance_report.example.json'));
...
const vectors = parseJSON(resolve(root, 'tests/non_collapse/non_collapse_vectors_v0.1.json'));
if (vectors) {
  ...
  check('30 total vectors', vectors.vectors.length === 30, `got ${vectors.vectors.length}`);
  check('10 rules defined', vectors.rules.length === 10, `got ${vectors.rules.length}`);
}

const goldenRuns = parseJSONL(resolve(root, 'tests/non_collapse/non_collapse_golden_runs_v0.1.jsonl'));
check('10 golden run traces', goldenRuns?.length === 10, `got ${goldenRuns?.length}`);

const negativeRuns = parseJSONL(resolve(root, 'tests/non_collapse/non_collapse_negative_runs_v0.1.jsonl'));
check('20 negative run traces', negativeRuns?.length === 20, `got ${negativeRuns?.length}`);

const variants = parseJSONL(resolve(root, 'tests/non_collapse/non_collapse_variants_v0.1.jsonl'));
check('10 variant vectors', variants?.length === 10, `got ${variants?.length}`);
