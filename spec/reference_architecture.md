...
**Receipt signature verification.** Every receipt carries an Ed25519 signature. An auditor retrieves the published public key and independently verifies the signature over the receipt's canonical JSON representation (the 19 signed fields). A valid signature proves the receipt was generated by the authorized Receipt Service and has not been modified. (Gateway-level tokens use ECDSA-secp256k1; see `spec/SIGNING_ALGORITHMS.md`.)
